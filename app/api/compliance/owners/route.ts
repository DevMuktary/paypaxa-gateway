...
export async function POST(request: Request) {
  try {
    ...
    const savedOwners = await prisma.compliance.upsert({
      where: { businessId: business.id },
      update: {
        ...
        dob: body.dob,
        bvn: body.bvn,
        bvnConsent: body.bvnConsent,
        livenessImage: body.livenessImage,
      },
      create: {
    ...
    return NextResponse.json({ success: true, data: savedOwners }, { status: 200 });
  } catch (error) {
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
