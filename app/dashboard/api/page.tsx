...
export default function ApiKeysPage() {
    ...
    const fetchKeys = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Initialize webhook inputs state
  ...
  const saveWebhook = async (keyId: string) => {
    ...
    try {
      const response = await fetch('/api/merchant/api-keys', {
        ...
        body: JSON.stringify({ keyId, webhookUrl: webhookInputs[keyId] })
      });
      if (response.ok) {
        // Show success briefly (in a real app, use a toast notification)
      }
    } catch (error) {
  ...
  const liveKey = data?.keys.find((k: any) => k.environment === 'LIVE'); // Won't exist until verified
  ...
  return (
    <div className="content-pad">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .btn-kyc { margin-top: 24px; background: var(--brand-primary); color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 14px; cursor: pointer; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3); transition: 0.2s; }
        .btn-kyc:hover { transform: translateY(-2px); }
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="key-section" style={{ borderTop: '4px solid #10B981', opacity: data.isLiveEnabled ? 1 : 0.8 }}>
            ...
            {!data.isLiveEnabled && (
              <div className="locked-overlay">
                <div className="locked-content">
                  ...
                  <button className="btn-kyc">Complete Verification</button>
                </div>
              </div>
            )}
            ...
            {liveKey ? (
              <>
                {/* Live keys would render here exactly like sandbox keys once generated */}
              </>
            ) : (
